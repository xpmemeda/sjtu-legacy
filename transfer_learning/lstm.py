...
def net(x):
    ...
    with tf.variable_scope("lstm"):
        input_lstm = tf.reshape(fc1, (-1, time_step, 128)) # shape = (?, 10, 3)
        cell = tf.contrib.rnn.BasicLSTMCell(50)
        initial_state = cell.zero_state(batch_size, dtype=tf.float32)
        output_lstm, final_states = tf.nn.dynamic_rnn(cell, input_lstm, initial_state=initial_state)
        tf.add_to_collection("output_RNN", output_lstm)
    # fc2:
    with tf.variable_scope("FC2"):
        input_fc2 = output_lstm[:, -1, :]
