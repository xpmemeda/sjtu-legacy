...
def net(x):
    ...
    with tf.variable_scope("MultiRNN"):
        input_MultiRNN = tf.reshape(fc1, (-1, time_step, 128))  # shape=(?, 10, 3)
        MultiRNN_cell = tf.nn.rnn_cell.MultiRNNCell(
            [tf.nn.rnn_cell.BasicRNNCell(num_units=50) for _ in range(3)])  # 生成 MultiRNN_Cell， 三层
        initial_state_MultiRNN = MultiRNN_cell.zero_state(batch_size, np.float32)  # 初始化
        output_MultiRNN, final_state_MultiRNN = tf.nn.dynamic_rnn(MultiRNN_cell, input_MultiRNN,
                                                                  initial_state=initial_state_MultiRNN)  # 计算结果，output 和 final_state
        # tf.add_to_collection("tmp", output_MultiRNN) 
    ...
    with tf.variable_scope("RNN"):
        input_RNN = output_MultiRNN
        RNN_cell = tf.nn.rnn_cell.BasicRNNCell(num_units=50)
        initial_state_RNN = RNN_cell.zero_state(tf.shape(output_MultiRNN)[0], tf.float32)
        output_RNN, final_state_RNN = tf.nn.dynamic_rnn(RNN_cell, input_RNN, initial_state=initial_state_RNN)
        tf.add_to_collection("output_RNN", output_RNN) 
    '''
    # fc:
    with tf.variable_scope("fc"):
        input_fc = tf.reshape(fc1, (-1, 10 * 128))
        w_fc = tf.Variable(tf.random_normal((10 * 128, 50)), name="w")
        b_fc = tf.Variable(tf.constant(0.1, shape=(1, 50)), name="b")
        wx_plus_b_fc = tf.add(tf.matmul(input_fc, w_fc), b_fc)
        fc = tf.nn.sigmoid(wx_plus_b_fc)
        tf.add_to_collection("output_RNN", fc)
