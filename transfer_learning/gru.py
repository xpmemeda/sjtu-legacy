...
def net(x):
    ...
    with tf.variable_scope("gru"):
        input_gru = tf.reshape(fc1, (-1, time_step, 128)) # shape = (?, 10, 3)
        cell = tf.contrib.rnn.GRUCell(50)
        initial_state = cell.zero_state(batch_size, dtype=tf.float32)
        output_gru, final_states = tf.nn.dynamic_rnn(cell, input_gru, initial_state=initial_state)
        tf.add_to_collection("output_RNN", output_gru)
    # fc2:
    with tf.variable_scope("FC2"):
        input_fc2 = output_gru[:, -1, :]
