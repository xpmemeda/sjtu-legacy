...
def getdata(time_step=time_step):
	...
	for i in range(time_step, dataset.shape[0]):
		...
		y = np.append(y, dataset[i-time_step+1:i+1])
		y_t = np.append(y_t, dataset[i])
	x = np.reshape(x, (-1, time_step, size))
	y = np.reshape(y, (-1, time_step, size))
	y_t = np.reshape(y_t, (-1, size))
	return x, y, y_t, Max, Min 
def get_a_cell():
    return tf.nn.rnn_cell.BasicRNNCell(num_units=50)
def net(x):
	...
	with tf.variable_scope("rnn"):
		...
		# print(cell)
		multi_cell = tf.nn.rnn_cell.MultiRNNCell([get_a_cell() for _ in range(3)])
	...
	#fc2:
	with tf.variable_scope("fc2"):
		input_fc2 = tf.reshape(output, (-1, 50))
		w_fc2 = tf.Variable(tf.random_normal((50, size)), name="w")
...
def train(batch_size=batch_size, time_step=time_step, train_start=0, train_end=7000, test_start=7000, test_end=10000):
	...
	y_ = tf.placeholder(tf.float32, shape=[None, time_step, size], name="label")
	...
	y_pre = tf.placeholder(tf.float32, shape=[None, size])
	loss_pre = tf.sqrt(tf.reduce_mean(tf.square(pred[:, -1, :] - y_pre)))
	...
	with tf.Session() as sess:
		...
		for epoch in range(iter_time):
			...
			if epoch%10==0:
				loss_train = sess.run(loss_pre, feed_dict={x_:x[train_start:train_end],
														   y_pre:y_t[train_start:train_end]})
				loss_test  = sess.run(loss_pre, feed_dict={x_:x[test_start:test_end], 
														   y_pre:y_t[test_start:test_end]})
